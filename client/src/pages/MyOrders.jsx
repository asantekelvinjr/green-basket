...
const MyOrders = () => {
  ...
  const filteredOrders = myOrders.filter((order) => {
    ...
    if (filter === "PENDING") return ["PLACED", "CONFIRMED"].includes(order.status);
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex gap-2">
            <button
              ...
              onClick={async () => {
                try {
                  const { data } = await api.put("/api/order/cancel", { orderId });
                  if (data.success) {
                    fetchMyOrders();
  ...
  return (
    <div className="mt-16 pb-16">
      ...
      {filteredOrders.length === 0 && (
        <p className="text-gray-400 text-center mt-16">No orders found.</p>
      )}
      ...
      {filteredOrders.map((order, index) => (
        <div
          ...
          onClick={() => { if (order.status !== "CANCELLED") navigate(`/my-orders/${order._id}`); }}
        >
          ...
          <div className="flex gap-3 mb-4 flex-wrap">
            ...
            <span className={`px-3 py-1 text-xs rounded-full ${getPaymentStyle(order.paymentStatus || "PENDING")}`}>
